...
const NavBar = () => {
  ...
  const navItems = [
    { to: "/recommend", label: "시장추천", icon: aiIcon },
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 flex justify-around items-center bg-white shadow-gray-100 h-[71px]"
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.to}
            to={item.to}
            className="flex flex-col items-center text-body4"
          >
            <img
              src={item.icon}
              alt={item.label}
